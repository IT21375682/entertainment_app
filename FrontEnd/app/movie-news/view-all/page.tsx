...
export default function PaginatedMovieNewsPage() {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      ...
      if (data.movies.length < limit || data.page >= data.totalPages) {
        setHasMore(false)
      } else {
        setHasMore(true)
      }
  ...
  return (
    <div className="min-h-screen bg-black text-white pt-16">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-6">All Movie News</h1>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {movies.map((post) => (
            <Link href={`/movie-news/${post._id}`} key={post._id}>
